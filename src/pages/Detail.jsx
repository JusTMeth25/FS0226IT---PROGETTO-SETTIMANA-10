...
const Detail = () => {
  ...
  if (loading) {
    return (
      <div className="detail-page text-center">
        ...
        <p className="mt-3 loading-text">Caricamento dati meteo per {decodeURIComponent(city)}...</p>
      </div>
    );
  }
  ...
  else if (weatherMain.includes("rain") || weatherMain.includes("drizzle")) weatherTypeClass = "weather-rain";
  else if (weatherMain.includes("snow")) weatherTypeClass = "weather-snow";
  else if (weatherMain.includes("thunder")) weatherTypeClass = "weather-thunderstorm";
  else if (weatherMain.includes("mist") || weatherMain.includes("fog") || weatherMain.includes("haze")) weatherTypeClass = "weather-mist";
        ...
        <div className="bg-decoration">
          ...
          {weatherTypeClass === "weather-rain" && (
            <>
              <div className="deco-cloud deco-cloud-lg"></div>
              {[...Array(8)].map((_, i) => (
                <div key={i} className="deco-rain-drop" style={{ left: `${10 + i * 11}%`, animationDelay: `${i * 0.15}s` }}></div>
              ))}
            </>
          )}
          {weatherTypeClass === "weather-snow" && (
            <>
              <div className="deco-cloud deco-cloud-md"></div>
              {[...Array(10)].map((_, i) => (
                <div key={i} className="deco-snowflake" style={{ left: `${5 + i * 10}%`, animationDelay: `${i * 0.3}s`, animationDuration: `${2 + (i % 3) * 0.7}s` }}></div>
              ))}
            </>
          )}
          {weatherTypeClass === "weather-thunderstorm" && (
            <>
              <div className="deco-cloud deco-cloud-lg" style={{ opacity: "0.7" }}></div>
              {[...Array(4)].map((_, i) => (
                <div key={i} className="deco-lightning" style={{ left: `${15 + i * 25}%`, animationDelay: `${i * 1.2}s` }}>⚡</div>
              ))}
              {[...Array(6)].map((_, i) => (
                <div key={i} className="deco-rain-drop" style={{ left: `${8 + i * 15}%`, animationDelay: `${i * 0.2}s` }}></div>
              ))}
            </>
          )}
          ...
          {isNight && (
            <>
              ...
              {[...Array(6)].map((_, i) => (
                <div key={i} className="deco-star" style={{ top: `${8 + i * 12}%`, left: `${60 + (i * 6) % 35}%`, animationDelay: `${i * 0.5}s` }}></div>
              ))}
            </>
          )}
        </div>
      ...
      <div className="detail-container">
        {cityImage && (
          <div className="city-banner">
            <img src={cityImage} alt={weather.name} className="city-banner-img" />
            <div className="city-banner-overlay"></div>
          </div>
        )}
        <div className="weather-card glass">
          <div className="weather-header">
            <div className="city-badge">{weather.sys.country}</div>
            <h1>{weather.name}</h1>
